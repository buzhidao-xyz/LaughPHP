...
class Image
{
	 ...
	 * 根据位置截取图片
	 * @param string $image  原图
	 * @param string $cimage 截取后的图片
	 * @param string $location 截取位置
	 * @param string $cwidth 要截取的图片宽度
	 * @param string $cheight 要截取的图片高度
	 * location : 'top','left','bottom','right','center' 
	 */
	static function cut($image,$cimage,$location='center',$cwidth=200,$cheight=150)
	{
		$return = false;
    ...
    $info = Image::getImageInfo($image);
		if ($info !== false) {
			...
			unset($info);
			$scale_w = $cwidth/$swidth;
			$scale_h = $cheight/$sheight;
			$scale = max($scale_w, $scale_h);
			$width = $cwidth;
			$height = $cheight;
			if ($scale < 1) {
				$srcwidth = round($swidth*$scale);
				$srcheight = round($sheight*$scale);
			} else {
				if ($scale_w > 1) $width = $srcwidth;
				if ($scale_h > 1) $height = $srcheight;
			}

			$src_x = round(($scale_w < 1) ? ($srcwidth-$cwidth)/2 : 0);
			$src_y = round(($scale_h < 1) ? ($srcheight-$cheight)/2 : 0);
			$dst_x = round(($scale_w < 1) ? 0 : ($cwidth-$srcwwidth)/2);
			$dst_y = round(($scale_h < 1) ? 0 : ($cheight-$srcheight)/2);
			...
			switch ($location) {
				...
				case 'left':
					...
					break;
				case 'bottom':
					$src_y = $srcheight-$height;
					break;
				case 'right':
					$src_x = $srcwidth-$width;
					break;
				case 'center':
					break;
				default:
			...
			//dump($location." ".$src_x." ".$src_y." ".$dst_x." ".$dst_y);exit;

			if (!$return) {
				// 载入原图
				$createFun = 'ImageCreateFrom' . ($type == 'jpg' ? 'jpeg' : $type);
				$simg = $createFun($image);
				$srcimg = imagecreatetruecolor($srcwidth, $srcheight);
                $background_color = imagecolorallocate($thumbImg, 0, 0, 0);  //  指定黑色
                imagecolortransparent($thumbImg, $background_color);  //  设置黑色为透明色，若注释掉该行则输出黑色的图
				imagecopyresampled($srcimg, $simg, 0, 0, 0, 0, $srcwidth, $srcheight, $swidth, $sheight);
				imagedestroy($simg);

				$dstimg = imagecreatetruecolor($cwidth, $cheight);
                $background_color = imagecolorallocate($thumbImg, 0, 0, 0);  //  指定黑色
                imagecolortransparent($thumbImg, $background_color);  //  设置黑色为透明色，若注释掉该行则输出黑色的图
				imagecopyresampled($dstimg, $srcimg, $dst_x, $dst_y, $src_x, $src_y, $width, $height, $width, $height);

				$imageFun = 'image' . ($type == 'jpg' ? 'jpeg' : $type);
				$imageFun($dstimg, $cimage);
				imagedestroy($dstimg);
				imagedestroy($srcimg);
